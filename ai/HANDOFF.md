...
**모델 폴백 — 임시조치 (2026-08-31, 호윤 지시 "임시조치만").**
`comet.py` 에 `_resolve()` 를 넣어 **설치 안 된 모델 이름이면 있는 걸로 갈아탄다.**
`_gen()` 의 `kw["model"]` 이 이걸 탄다. 순서: ① 그대로 있으면 그대로 → ② 같은 계열
(`gemma4:99b`→`gemma4:26b`) → ③ TIERS 에 적힌 모델 중 깔린 것 → ④ 임베딩 아닌 아무거나.
...
검증: `gemma3:4b`→`gemma4:12b` · `qwen2.5:32b`→`gemma4:12b` · `gemma4:99b`→`gemma4:26b`,
없는 이름(`gemma3:27b`)으로 `_gen()` 을 불러도 답이 나온다. 문지기 정상 동작 재확인.
- 호흡: 등급별 keep_alive로 무거운 모델은 곧 VRAM에서 자동 언로드.
